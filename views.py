...
def reports():
    ...
    total_cost = WorkorderItem.select(fn.sum(WorkorderItem.cost))\
                            .join(Workorder, on=(WorkorderItem.workorder_id == Workorder.id))\
                            .where((Workorder.paid) & (fn.DATE(Workorder.paid_date).between(date_start, date_end))).get()
    ...
    if(total_cost.cost != None):
        profits = workorder_stats['subtotal'] - total_cost.cost
        profit_margin = profits / workorder_stats['subtotal']
